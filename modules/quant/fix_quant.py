...
def calculate_qparams(x, num_bits, flatten_dims=_DEFAULT_FLATTEN, reduce_dim=0, reduce_type='mean', keepdim=False,
    ...
    with torch.no_grad():
        ...
        if x_flat.dim() == 1:
            ...
            min_values = _deflatten_as(x_flat.min(-1)[0], x)  # 找到最小值和最大值 
...
class UniformQuantize(InplaceFunction):
    ...
    def forward(ctx, input, num_bits=None, qparams=None, flatten_dims=_DEFAULT_FLATTEN,
        ...
        scale = qparams.range / (qmax - qmin) # scale 实际上是一个 fp32

        min_scale = torch.tensor(1e-8).expand_as(scale).cuda() # 设置了一个最小值
        scale = torch.max(scale, min_scale)

        with torch.no_grad():
            output.add_(qmin * scale - zero_point).div_(scale) # 这里才进行实际的量化
            if stochastic:
                noise = output.new(output.shape).uniform_(-0.5, 0.5)
                output.add_(noise)
            # quantize
            output.clamp_(qmin, qmax).round_() # 直接取round 
...
class QConv2d(nn.Conv2d):
    ...
    def forward(self, input, num_bits, num_grad_bits):
        ...
        qinput = self.quantize_input(input, num_bits) # input取平均值，而weight取最值
        output = F.conv2d(qinput, qweight, qbias, self.stride, self.padding, self.dilation, self.groups) # 累加起来还是 8 位吗？
        output = quantize_grad(output, num_bits=num_grad_bits, flatten_dims=(1, -1)) # 还需要对于反向传播的梯度进行量化
...
if __name__ == '__main__':
    ...
    x_q = quantize(x, flatten_dims=(1, -1), num_bits=8, dequantize=True)
    print(x)
    print(x_q)