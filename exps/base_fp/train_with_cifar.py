...
parser = argparse.ArgumentParser(description="SGD/SWA training on CIFAR")
parser.add_argument(
...
def make_number(number, exp=-1, man=-1):
    if number == "float":
...
for num in num_types:
    ...
    number_dict[num] = make_number(
        num_type, exp=num_exp, man=num_man
    )
