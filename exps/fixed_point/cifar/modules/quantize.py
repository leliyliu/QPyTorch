...
class UniformQuantize(InplaceFunction):
    ...
    def forward(ctx, input, num_bits=None, qparams=None, flatten_dims=_DEFAULT_FLATTEN,
        ...
        min_scale = torch.tensor(1e-8).expand_as(scale).cuda() # 可能是由于范围过大导致了相应的问题
        ...
        with torch.no_grad():
            if qparams is None:
                ...
                qparams = calculate_qparams(
                    ...
                    reduce_type='extreme', percentage=0.9)
...
class QConv2d(nn.Conv2d):
    ...
    def __init__(self, in_channels, out_channels, kernel_size,
        ...
        # self.quantize_input = QuantMeasure(shape_measure=(1, 1, 1, 1), flatten_dims=(1, -1), stochastic=True)
    ...
    def forward(self, input, num_bits, num_grad_bits):
        ...
        if self.bias is not None:
            ...
            #     flatten_dims=(0, -1), stochastic=True)
            qbias = quantize(
                self.bias, num_bits=self.num_bits,
                flatten_dims=(0, -1))
        else:
        ...
        weight_qparams = calculate_qparams(self.weight, num_bits=num_bits, flatten_dims=(1, -1),
                                           reduce_dim=None)
        # qweight = quantize(self.weight, qparams=weight_qparams, stochastic=True)
        ...
        output = quantize_grad(output, num_bits=num_grad_bits, flatten_dims=(1, -1), stochastic=False)
...
if __name__ == '__main__':
    ...
    qparams = calculate_qparams(x, 8, flatten_dims=_DEFAULT_FLATTEN)
    print(qparams)
    qparams = calculate_qparams(x, 8, flatten_dims=_DEFAULT_FLATTEN_GRAD)
    print(qparams)