...
from qtorch.number import FAST_BFP
from quant import *

num_types = ["weight", "activate", "error", "acc", "grad", "momentum"]

parser = argparse.ArgumentParser(description='FAST SGD training')
...
parser.add_argument('--tile', type=int, default=16, choices=[16, 24, 32, 48, -1, 0],
...
for i, num in enumerate(num_types):
    ...
    number_dict[num] = FAST_BFP(exp=num_exp, man=num_man, 
            group=args.tile)
